...
-- Role programs the Bank hands out but does not run: fetched when a client
-- first installs one, and kept in memory (see depot below).
RELEASE.depot_files = {
    "pumpe.lua",
    "revolution.lua",
    "bank_app_server.lua",
...
-- Easy Deployment's files, 11.2 -------------------------------------------------
-- A client installing a role over Rednet is handed its files from here.
-- The Bank's own runtime is on its disk anyway; every other role program is
-- fetched from the published release the first time somebody installs it.
-- Until 11.2 each one was then kept in /updates on this computer's disk --
-- the PUMPE's program alone is a quarter of it -- and that was half of how
-- a Core ran out of room. They are kept in memory now, a few at a time, and
-- a restart simply starts the cache again. Nothing here writes to the disk.
...
-- What clients are given as config.lua: this Bank's settings without the
-- government key.
...
-- What earlier releases left on the disk: the /updates cache, and staging
-- from an update that never finished. All of it can be fetched again, and
-- on a full Core it is the room the Bank needs to start.
...
local function blankState()
    return {
        ...
        companies = {},
        terminals = {},
        -- 11.2: history and notifications on their way to the Vault.
        outbox = {},
        tx_count = 0,
        declaration_periods = {},
        declarations = {},
        ...
        -- tickets and app records have been the Vault's since 9.3.
        proximity_offers = {},
        developers = {},
        announcements = {},
...
local function ensureState()
    ...
    -- Territory names, visa codes, visit statuses and event ticket lists
    -- are repaired on the Vault, where those records live.
    state.schema = 9
...
-- A save writes the whole file beside the old one before swapping them, so
-- it needs as much free space as the file is big. If that is not there, the
-- history still waiting for the Vault is the one thing in here that is not
-- money: it goes, rather than the money with it.
local function save()
...
-- Records the Vault keeps, 11.2 ------------------------------------------------
-- A bank remembers two things for ever unless it is told not to: what
-- happened to the money, and what it told people about it. Until 11.2 both
-- lived here, in the one file that also holds the money -- three thousand
-- transactions and fifty notifications an account -- and between them they
-- filled the Core's disk until it could not start.
--
-- They are the Vault's now. The Core keeps only what money needs: balances,
-- an unread count and the newest notification for the phone's poll, and a
-- day-by-day income tally for tax. Everything else goes down the cable
-- through an outbox that is saved with the money, so a Vault that is away,
-- or still on an older release, loses nothing: the records wait here and go
-- when it answers. The Vault drops repeats, so sending twice is harmless.
local records = {
    batch = 60,
    cap = 1500,
    -- Tax periods are counted in days; this is how far back the tally goes.
    income_days = 120,
...
function records.push(kind, accountId, item)
    ...
    outbox[#outbox + 1] = { s = state.outbox_seq, k = kind, a = accountId,
        i = item }
    -- Only while the Vault is away for a long time. What goes is the oldest
    -- history; balances are never in here.
    while #outbox > records.cap do table.remove(outbox, 1) end
end

-- After the Vault fails to answer, a while in which nothing here waits on
-- it again: a phone opening Activity is served what is waiting at once
-- rather than after two timeouts in a row.
function records.away()
    return records.down_until ~= nil and util.nowMs() < records.down_until
end

-- Sends what is waiting, a batch at a time. False when the Vault did not
-- take it; the records stay for the next try.
function records.flush()
    ...
    while #state.outbox > 0 do
        ...
        -- Items pushed while this batch was on the cable are behind it.
...
-- Income, by the day it arrived. What a tax period needs, without keeping
-- every transaction to add up.
...
-- 11.1 and earlier kept all of it here. The newest of each, per account --
-- as many as the Vault keeps -- go to the Vault; the rest were never going
-- to be shown again. Once moved there is nothing left, so this is free on
-- every later start.
do
...
-- Read from the Vault, with whatever has not gone down the cable yet on
-- top. A Vault that is away still shows the newest, from the outbox.
local function readRecords(account, kind)
    ...
    -- Read or not is the Core's to say: marking read happens here first.
...
-- actually asks to install one, then kept in memory while clients need them.
